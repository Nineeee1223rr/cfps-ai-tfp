...
1. **识别策略**：受限于环境，本项目采用 T-learner 作为因果森林的替代实现。未来可在支持 C++ 编译的环境下使用官方 `grf` 或 `econml` 库进行更精确的异质性效应估计。
2. **变量测度**：`AI_Index` 主要基于互联网使用频率等代理指标，未来可结合更细致的数字化应用数据（如智能农机使用、电商参与等）以增强指标的解释力。
3. **样本局限**：由于变量缺失，部分年份有效样本量有所下降，结果的稳健性仍需在更长时段、更大样本下进一步检验。
4. **内生性处理**：目前的分析虽采用了 PSM 和机器学习方法，但仍可能存在未观测到的遗漏变量。未来研究可尝试寻找合适的工具变量以增强因果识别。
