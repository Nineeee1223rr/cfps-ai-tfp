...
def build_year_df(spec: YearSpec) -> pd.DataFrame:
    """构建单年份的家庭农业经营数据集，包含产出(Y)与要素投入(K, M, L)"""
...
def compute_tfp(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 劳动投入 L 采用雇工成本，对未雇工家庭做截断处理 (ln(1)=0)
    df["lnL"] = _positive_log(df["L_cost"].fillna(0) + 1.0)

    # 剔除极端离群值，避免影响 OLS 估计残差（lnTFP）的稳健性
    # 仅针对回归所需的四个核心变量进行去极值处理
    for col in ["lnY", "lnK", "lnM"]:
        if col in df.columns:
            v = df[col].dropna()
            if not v.empty:
                low, high = v.quantile([0.01, 0.99])
...
def main():
    ...
    # PSM估计结果与T-Learner存在差异的原因分析：
    # 1. PSM (ATT=-0.004) 衡量的是在控制了可观测变量后的净效应，由于样本选择偏差被控制，且农业生产受自然环境等非数字化因素影响较大，净效应可能较小且不显著。
    # 2. T-Learner (ATE=0.071) 基于随机森林估计，能够捕捉非线性关系与变量间的交互效应，因此在反映AI应用的潜在提升潜力时表现出更高的估计值。
    dpsm = df.copy()
